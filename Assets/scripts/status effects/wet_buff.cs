...
//[CreateAssetMenu(fileName = "StatusEffectTemplate", menuName = "status effect template")] //note, this may only let you add copies of this script via scriptable objects.. so you might just have to copy and paste this script a lot instead.
    public class wet_buff : MonoBehaviour
    {
        ...
        public float water_element_multiplier = 1.5f;
        
        public void Start()
        {
            if (moved_version == false)
            {
                if (this.transform.parent.TryGetComponent(out wet_buff this_script)) //change statuseffecttemplate out for the name of the script.
                {
                    this_script.add();
                }
                else
                {
                    transform.parent.AddComponent<wet_buff>().moved_version = true;
                }
            ...
            else
            {
                ...
                effect_display_prefab = StatusEffectAdder.current.statusEffect_displays[1];//replace with whatever number is associated with this effect
